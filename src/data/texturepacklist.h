...
class TexturePackList : public std::vector<TexturePack>
{
public:
	TexturePackList(const wxString& dir = wxEmptyString);

	// Reloads the world list.
	virtual void UpdateTexturePackList();
	...
	TexturePack *FindByFilename(const wxString& filename);
	...
	bool DeletePack(size_t index);

protected:
	// Loads the save list from the given directory.
	// Returns true if the list changed.
	virtual bool LoadTexturePackListFromDir(const wxString& loadFrom = wxEmptyString);
